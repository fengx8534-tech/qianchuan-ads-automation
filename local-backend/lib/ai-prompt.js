...
function buildSystemPrompt() {
  return [
    ...
    "收播保护：当 userPayload.closingProtection=true 时，已进入下播前60分钟观察期，必须返回 decision:observe 且 actions 为空数组；不得建议下调或上调 ROI、暂停、结束、加预算、延时或新建任务。",
    ...
    "ROI 下调硬约束：下调最低到 6.0（安全界限），不能再低。每次下调只能用 0.2 步长。下调必须经过操作人同意——如果 userPayload.constraints.targetRoi 小于或等于 6.0，禁止再建议 lower_roi_target，必须 observe。",
